...
class OnChainAnalyzer:
    ...
    def get_token_holders(self, mint_address: str) -> OnChainHolderAnalysis:
        """
        Get token holders using InsightX API (best) or fallback to Helius RPC
        """
        try:
            print(f"[ONCHAIN] Getting holders for {mint_address[:8]}...")

            # Try InsightX first (has total holder count + top holders)
            insightx_data = self._get_holders_from_insightx(mint_address)
            if insightx_data:
                return insightx_data

            # Fallback to Helius RPC
            print(f"[ONCHAIN] InsightX not available, using Helius RPC...")
            ...
            # Get largest token holders using Helius (faster and more reliable)
            print(f"[ONCHAIN] Step 2: Getting largest token holders...")
            payload = {
                "jsonrpc": "2.0",
                "id": 1,
            ...
            print(f"[ONCHAIN] Found {len(accounts)} largest holders")

            # Parse holders from getTokenLargestAccounts response
            holders = []
            for account in accounts:
                try:
                    address = account.get("address", "")
                    balance = float(account.get("uiAmount", 0))
                    ...
                    print(f"[ONCHAIN] Error parsing holder: {e}")
                    continue

            # Already sorted by balance (getTokenLargestAccounts returns sorted)
    ...
    def _get_holders_from_insightx(self, mint_address: str) -> Optional[OnChainHolderAnalysis]:
        """Get holders from InsightX API (best source with total count)"""
        try:
            from insightx_api import InsightXAPI

            print(f"[ONCHAIN] Trying InsightX API...")
            insightx = InsightXAPI()
            scan_data = insightx.scan_token(mint_address)
            insightx.close()

            if not scan_data or 'results' not in scan_data:
                return None

            advanced = scan_data['results'].get('advanced', {})
            holder_count = advanced.get('holder_count', 0)
            top_holders_data = advanced.get('top_holders', [])

            if not top_holders_data:
                return None

            print(f"[ONCHAIN] InsightX: Found {holder_count} total holders, {len(top_holders_data)} top holders")

            # Get token supply
            supply = self._get_token_supply(mint_address)
            if not supply:
                supply = 1000000000  # Default 1B for percentage calculation

            # Get liquidity pool addresses to exclude from holder analysis
            liquidity_pools = set()
            locked_liq = advanced.get('locked_liquidity', [])
            for lock in locked_liq:
                pool_addr = lock.get('pair_address', '')
                if pool_addr:
                    liquidity_pools.add(pool_addr)

            # Parse holders (exclude liquidity pools)
            holders = []
            excluded_pools = []
            for holder_data in top_holders_data:
                address = holder_data.get('address', '')
                balance = float(holder_data.get('balance', 0))
                percentage = (balance / supply) * 100 if supply > 0 else 0

                # Skip liquidity pools
                if address in liquidity_pools:
            ...
            print(f"[ONCHAIN] Excluded {len(excluded_pools)} liquidity pool(s) from holder analysis")

            # Calculate metrics (use actual holder count from InsightX)
            total_holders = holder_count  # Real total, not just top 20!
            top_holder_pct = holders[0]["percentage"] if holders else 0
            top_3_pct = sum(h["percentage"] for h in holders[:3])
            top_10_pct = sum(h["percentage"] for h in holders[:10])

            # Analyze top holders (simplified for InsightX - skip age check to avoid extra RPC calls)
            fresh_count_top10 = 0
            fresh_count_top20 = 0
            exchanges_count = 0
            holders_selling = []
            ...
            # Use default age since we can't check each wallet (would be too slow)
            avg_age = 30  # Assume average age

            # Detect coordinated exit
            coordinated_exit = len(holders_selling) >= 3

            # Detect whale dumping
            whale_dumping = len(holders_selling) >= 2 and top_holder_pct > 10

            # Calculate risk
            risk_score, red_flags = self._calculate_risk_and_flags(
                holders, top_holder_pct, top_3_pct, top_10_pct,
                exchanges_count, fresh_count_top10, len(holders_selling),
                coordinated_exit, whale_dumping, avg_age
            )
            ...
            print(f"[ONCHAIN] InsightX error: {e}")
            return None
