...
class TopHoldersAnalyzer:
    ...
    def _get_holders_from_rpc(self, mint_address: str) -> List[Dict]:
        """
        Get top holders using Solscan API
        Faster and more reliable than RPC calls
        """
        try:
            # Use Solscan API to get holders
            from solscan_api import SolscanAPI

            solscan = SolscanAPI()
            holders_data = solscan.get_token_holders(mint_address, limit=100)
            solscan.close()

            if not holders_data:
                return []

            # Convert Solscan format to our format
            holders = []
            ...
            print(f"[!] Error getting holders from Solscan: {e}")
            return []
